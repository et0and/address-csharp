...
public record struct ClientOptions()
{
    ...
    Lazy<string> _baseUrl = new(() =>
        Environment.GetEnvironmentVariable("ADDRESS_BASE_URL") ?? EnvironmentUrl.Production
    );
    ...
    /// <see cref="IAddressClient.WithRawResponse"/>, don't perform validation until deserialization
    /// is triggered.</para>
    /// </summary>
    ...
    Lazy<string> _apiKey = new(() =>
        Environment.GetEnvironmentVariable("ADDRESS_API_KEY")
        ?? throw new AddressInvalidDataException(
