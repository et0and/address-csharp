# Address C# API Library

The Address C# SDK provides convenient access to the [Address REST API](https://address.tom.so/docs) from applications written in C#.

It is generated with [Stainless](https://www.stainless.com/).

The REST API documentation can be found on [address.tom.so](https://address.tom.so/docs).
...
// Configured using the ADDRESS_API_KEY and ADDRESS_BASE_URL environment variables
AddressClient client = new();
```
...
AddressClient client = new() { ApiKey = "My API Key" };
```
...
| --------- | -------------------- | -------- | -------------------------- |
| `ApiKey`  | `ADDRESS_API_KEY`    | true     | -                          |
| `BaseUrl` | `ADDRESS_BASE_URL`   | true     | `"https://address.tom.so"` |
...
To send a request to the Address API, build an instance of some `Params` class and pass it to the corresponding client method. When the response is received, it will be deserialized into an instance of a C# class.

For example, `client.Health.Check` should be called with an instance of `HealthCheckParams`, and it will return an instance of `Task<HealthCheckResponse>`.
...
```csharp
var response = await client.WithRawResponse.Health.Check();
...
var response = await client.WithRawResponse.Health.Check();
HealthCheckResponse deserialized = await response.Deserialize();
...
- `AddressApiException`: Base class for API errors. See this table for which exception subclass is thrown for each HTTP status code:

| Status | Exception                              |
| ------ | -------------------------------------- |
| 400    | `AddressBadRequestException`           |
| 401    | `AddressUnauthorizedException`         |
| 403    | `AddressForbiddenException`            |
| 404    | `AddressNotFoundException`             |
| 422    | `AddressUnprocessableEntityException`  |
| 429    | `AddressRateLimitException`            |
| 5xx    | `Address5xxException`                  |
| others | `AddressUnexpectedStatusCodeException` |

Additionally, all 4xx errors inherit from `Address4xxException`.

- `AddressIOException`: I/O networking errors.

- `AddressInvalidDataException`: Failure to interpret successfully parsed data. For example, when accessing a property that's supposed to be required, but the API unexpectedly omitted it from the response.

- `AddressException`: Base class for all exceptions.
...
AddressClient client = new() { MaxRetries = 3 };
```
...
AddressClient client = new() { Timeout = TimeSpan.FromSeconds(42) };
```
...
AddressClient client = new() { HttpClient = httpClient };
```
...
var parameters = RequestKeyCreateParams.FromRawUnchecked
(
...
By default, the SDK will not throw an exception in this case. It will throw `AddressInvalidDataException` only if you directly access the property.
...
AddressClient client = new() { ResponseValidation = true };
```
